	...
	this.div.qtip({
		...
		show: {
		    ...
		    effect: function(offset) {
				$(this).fadeIn();
				for(var i=0; i<enemies.length; i++){ enemies[i].updateQtip(); }
			}
		},
		hide: {
			event: 'unfocus',
...
*/	
Enemy.prototype.defeated = function(killer){
	var recruitmentChance = this.recruitmentChance;
	//increase if killer was Friendly (increases chances)
	if(killer && killer.ability == Abilities.Friendly)
		recruitmentChance *= killer.FRIENDLY_RECRUITMENT_MULTIPLIER;
		
	if(pushLuck(recruitmentChance))
		this.recruit();
	else
		this.die();
}
...
Enemy.prototype.takeTurn = function(target){
	...
	if(this.ability == Abilities.Sniper){
		...
		if(distance <= this.range && isInt(getRawDistance(this,target)) /* can't attack diagonally */){
			...
			var direction = getPreferredDirection(this,target);
...
Enemy.prototype.updateQtip = function(){
	...
	var healthClass = 'ui-widget-header'; //'hp-healthy'; //DON'T show green for enemy
	if(this.getHPPercent() <= HEALTH_RED_THRESHOLD){
		healthClass = 'ui-state-error';
	}	
	
	//if shiny, add star next to stats
	var shinyStar = '';
	if(this.isShiny())
	...
	this.div.qtip('option',{
		'content.text': '<div class="ui-progressbar ui-widget ui-widget-content ui-corner-all" style="width: 100%;height:15px;">' +
				...
				'<div style="width: ' + Math.round(this.getHPPercent() * 100) + '%;" class="ui-progressbar-value ui-corner-left hp-bar-content ' + healthClass + '"></div></div> ' +
