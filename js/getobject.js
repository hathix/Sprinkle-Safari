...
/**
	Finds the enemy who is closest to an ally. Enemies that are stunned are excluded.
	@return [Enemy] the enemy who is the closest to an ally out of all the enemies. Returns null if no enemy is eligible to move (they're all stunned).
*/
function getEnemyClosestToAlly(){
	var closestEnemy = null;
	var shortestDistance = 100; //some arbitrarily huge number
	
	for(var i=0; i<enemies.length; i++){
		var enemy = enemies[i];
		if(enemy.flags.getFlag(FLAG_STUNNED) == true || enemy.flags.getFlag(FLAG_LOCKDOWN_TURNS_LEFT) > 0){
			//they're stunned, ignore them
			continue;
		}
		
		//find distance between this enemy and the ally closest to it
...
/**
 * Returns an array of all the enemies of the given animal who are just 1 space away; these are the opposing neighbors.
 * @param	{Animal}	viewer	the animal to check the enemies of
 * @return	{Animal[]}}	every animal on the opposite team that's next to viewer, in any order
 */
function getAdjacentEnemies(viewer){
	var allEnemies = getFoes(viewer);
	var neighbors = [];
	for(var i=0; i<allEnemies.length; i++){
		if(getDistance(viewer, allEnemies[i]) == 1){ //if it's 0 we're looking at this exact animal
			neighbors.push(allEnemies[i]);
		}
	}
